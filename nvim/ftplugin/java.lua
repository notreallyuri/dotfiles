...
vim.list_extend(bundles, vim.tbl_filter(function(jar)
  return not jar:match("com%.microsoft%.java%.test%.runner%-jar%-with%-dependencies%.jar$")
end, vim.fn.glob(mason .. "/packages/java-test/extension/server/*.jar", true, true)))
...
jdtls.start_or_attach({
  ...
  on_attach = function(_, buf)
    ...
    vim.keymap.set("v", "<leader>cv", function() jdtls.extract_variable(true) end,
      { buffer = buf, desc = "Extract variable (Java)" })
    vim.keymap.set("v", "<leader>cC", function() jdtls.extract_constant(true) end,
